...
begin
  ...
  try
    ANode:=AXML.Add('Test');
    ANode.Attrs.AsString['Name']:='QDAC';
    ANode.Attrs.AsInt['Age']:=3;
    ANode.Attrs.AsBoolean['OpenSource']:=True;
    ANode.Attrs.AsDateTime['LastBuild']:=Now;
    ANode.Attrs.AsFloat['Version']:=3.0;
    ANode.Attrs.AsGuid['Id']:=NewId;
    ANode.Attrs.AsFloat['Version']:=3.1;
//    ANode.Attrs.Add('Name','QDAC');
//    ANode.Attrs.Add('Age',3);
//    ANode.Attrs.Add('OpenSource',true);
//    ANode.Attrs.AddDateTime('LastBuild',Now);
//    ANode.Attrs.Add('Version',3.0);
//    ANode.Attrs.Add('Id',NewId);
    mmResult.Lines.Add('Name='+ANode.Attrs.ValueByName('Name'));
    mmResult.Lines.Add('Age='+IntToStr(ANode.Attrs.IntByName('Age',0)));
    mmResult.Lines.Add('OpenSource='+BoolToStr(ANode.Attrs.BoolByName('Age',false),true));
    mmResult.Lines.Add('LastBuild='+DateTimeToStr(ANode.Attrs.DateTimeByName('LastBuild',0)));
    mmResult.Lines.Add('Version='+FloatToStr(ANode.Attrs.FloatByName('Version',0)));
    mmResult.Lines.Add('Id='+GuidToString(ANode.Attrs.GuidByName('Id',Guid_Null)));
    ANode:=ANode.AddNode('Node').AddText('Hello,world');
    ShowMessage(ANode.Path);
  finally
    FreeObject(AXML);
  end;
