...
begin
  ...
  begin
    ...
    begin
      ABuilder.Current := pd;
      ABuilder.IncSize(Length(S) - ((IntPtr(p) - IntPtr(ps)) shr 1));
      pd := ABuilder.Current;
      ps := p;
    end;
    case p^ of
      ...
      begin
        ...
        begin
          ...
          Inc(pd, 2);
          pd^ := HexChars[(PWord(p)^ shr 12) and $0F];
          Inc(pd);
          pd^ := HexChars[(PWord(p)^ shr 8) and $0F];
          Inc(pd);
          pd^ := HexChars[(PWord(p)^ shr 4) and $0F];
          Inc(pd);
          pd^ := HexChars[PWord(p)^ and $0F];
          Inc(pd);
        end;
      end;
    end;
