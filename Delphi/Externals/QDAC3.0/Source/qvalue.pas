...
var
  ...
  begin
    AType := TYPE_FLOAT32;
    AStream.WriteBuffer(AType, 1);
    AStream.WriteBuffer(Value.AsSingle, 4);
  end;
  ...
  begin
    AType := TYPE_FLOAT64;
    AStream.WriteBuffer(AType, 1);
    AStream.WriteBuffer(Value.AsFloat, 8);
  end;
  ...
  begin
    AType := TYPE_GUID;
    AStream.WriteBuffer(AType, 1);
    AStream.WriteBuffer(Value.AsGuid^, 16);
  end;
  procedure WriteBcd;
  begin
    AType := TYPE_BCD;
    AStream.WriteBuffer(AType, 1);
    AStream.WriteBuffer(Value.AsBcd^, SizeOf(TBcd));
  end;

  procedure WriteInterval;
  begin
    AType := TYPE_INTERVAL;
    AStream.WriteBuffer(AType, 1);
    AStream.WriteBuffer(Value.AsInterval, SizeOf(TQInterval));
  end;
  ...
  begin
    ...
    else if SameValue(Value.AsDateTime, V) then // Date Only
    begin
      AType := TYPE_DATE;
      AStream.WriteBuffer(AType, 1);
    ...
    begin
      AType := TYPE_DATETIME;
      AStream.WriteBuffer(AType, 1);
      AStream.WriteBuffer(Value.AsDateTime, SizeOf(TDateTime));
    end;
  end;

  procedure WriteCurrency;
  begin
    if (Value.AsInt64 >= 0) and (Value.AsInt64 <= 15) then
    begin
      AType := TYPE_MONEY4 + Byte(Value.AsInt64);
      AStream.WriteBuffer(AType, 1);
    end
    else if (Value.AsInt64 >= -128) and (Value.AsInt64 <= 127) then // 1B
    begin
      AType := TYPE_MONEY8;
      AStream.WriteBuffer(AType, 1);
      AStream.WriteBuffer(Value.AsInt64, 1);
    end
    else if (Value.AsInt64 >= -32768) and (Value.AsInt64 <= 32767) then // 2B
    begin
      AType := TYPE_MONEY16;
      AStream.WriteBuffer(AType, 1);
      AStream.WriteBuffer(Value.AsInt64, 2);
    end
    else if (Value.AsInt64 >= -2147483648) and (Value.AsInt64 <= 2147483647)
    then // 4B
    begin
      AType := TYPE_MONEY32;
      AStream.WriteBuffer(AType, 1);
      AStream.WriteBuffer(Value.AsInt64, 4);
    end
    else
    begin
      AType := TYPE_MONEY64;
      AStream.WriteBuffer(AType, 1);
      AStream.WriteBuffer(Value.AsInt64, 8);
    end;
  end;
  ...
  var
    ...
    S: QStringA;
  begin
    S := qstring.Utf8Encode(Value.AsString^);
    L := S.Length;
    if L < (Length(Value.AsString^) shl 1) then // UTF8编码更短一些
    begin
      if L < 16 then // 0B
      begin
    ...
    begin
      ...
      AStream.WriteBuffer(PQCharW(Value.AsString^)^, L);
    end;
  end;
  ...
  begin
    ...
    AStream.WriteBuffer(TQValueStream(Value.AsStream).Memory^, L);
  end;
...
begin
  ...
  case ValueType of
    ...
    vdtFloat:
      WriteFloat;
    vdtInteger:
      WriteInt(Value.AsInteger);
    vdtInt64:
      WriteInt(Value.AsInt64);
    vdtCurrency:
      WriteCurrency;
    vdtBcd:
      WriteBcd;
