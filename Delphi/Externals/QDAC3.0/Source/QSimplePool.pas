...
begin
  ...
  begin
    TQSimplePoolNewItemEventA(TMethod(FOnNewItem).Code) := AOnNew;
    TMethod(FOnNewItem).Data := Pointer(-1);
  end;
  if Assigned(AOnFree) then
  begin
    TQSimplePoolItemNotifyA(TMethod(FOnFree).Code) := AOnFree;
    TMethod(FOnFree).Data := Pointer(-1);
  end;
  if Assigned(AOnReset) then
  begin
    TQSimplePoolItemNotifyA(TMethod(FOnReset).Code) := AOnReset;
...
{$IFDEF UNICODE}
  ...
  begin
    if TMethod(FOnNewItem).Data = Pointer(-1) then
      TQSimplePoolNewItemEventA(TMethod(FOnNewItem).Code) := nil;
  end;
  if Assigned(FOnFree) then
  begin
    if TMethod(FOnFree).Data = Pointer(-1) then
      TQSimplePoolItemNotifyA(TMethod(FOnFree).Code) := nil;
  end;
  if Assigned(FOnReset) then
  begin
    if TMethod(FOnReset).Data = Pointer(-1) then
      TQSimplePoolItemNotifyA(TMethod(FOnReset).Code) := nil;
  end;
{$ENDIF}
  inherited;
end;
...
{$ENDIF}
  ...
  else if not Assigned(FOnNewItem) then
