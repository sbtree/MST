...
var
  ...
  AStamp: Int64;
  APlan: PQJobPlanData;
  function CheckPlan: TQPlanTimeoutCheckResult;
  begin
    if AStamp > APlan.NextTime then // 检查任务是否错过执行
    begin
      if AItem.PopTime < APlan.NextTime then // 错过了，立即安排执行
      begin
        Result := pcrOk;
        Exit;
      end
      ...
      begin
        Result := APlan.Plan.Timeout(ATime);
        if Result <> pcrTimeout then
          APlan.NextTime := AStamp + MilliSecondsBetween(APlan.Plan.NextTime,
            ATime) * 10;
      end;
    end
    else
...
begin
  FWorkJob.WorkerProc.ForProc := AWorkerProc;
  Start;
  Wait(AMsgWait);
end;
...
begin
  ...
  APlanData.NextTime := ATimeStamp + MilliSecondsBetween(APlan.NextTime,
    Now) * 10;
  if AData <> nil then
  begin
    if AFreeType = jdfFreeAsInterface then
