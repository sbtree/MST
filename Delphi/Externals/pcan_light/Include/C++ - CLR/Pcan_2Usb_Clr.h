...
//  - CAN_ReadExt([Out]TPCANMsg %msg, [Out]TPCANTimestamp %timestamp)
//  - VersionInfo(StringBuilder^ buffer)
//  - CAN_DLLVersionInfo(StringBuilder^ buffer)
...
//  - ResetFilter()
//	- SetUSBDeviceNr(unsigned int DevNum) 
//	- GetUSBDeviceNr(unsigned int %DevNum) 
		...
		namespace Light
		{
			public ref class PCAN_2USB
			{
				public:
					...
					static unsigned int SetUSBDeviceNr(unsigned int DevNum);
					...
					static unsigned int GetUSBDeviceNr(unsigned int %DevNum);
